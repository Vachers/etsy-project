...
export default function DigitalBundlesPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch("/api/products?category=digital-bundles");
        if (res.ok) {
        ...
        console.error("Digital bundles yüklenirken hata:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <ProductPageLayout
      ...
      description="Dijital paketlerinizi yönetin"
      category="digital-bundles"
      products={products}
